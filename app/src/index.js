...
const App = {
  ...
  start: async function () {
    ...
    try {
      ...
      this.demotoken = new web3.eth.Contract(
        DemoToken.abi,
        DemoToken.networks[networkId].address
      );
      // get accounts
      ...
      try {
        await this.approveAmount(deployedNetwork.address, 1000000000000);
      } catch (err) {
        console.log(err);
      }
      this.refreshBalance();
  ...
  refreshBalance: async function () {
    ...
    const userbalanceElement = document.getElementsByClassName(
      "user_balance"
    )[0];
  ...
  getToken: async function () {
    ...
    const did = parseInt(
      document.getElementById("unstake_did").value
    ).toString();
  ...
  approveAmount: async function (spender, amount) {
    const { approve } = this.demotoken.methods;
    let hasUserApproved = localStorage.getItem("hasUserApproved");
    if (hasUserApproved === null) {
      await approve(spender, amount).send({ from: this.account });
      localStorage.setItem("hasUserApproved", true);
    } else {
      console.log("User already approved to spend tokens");
    }
  },
